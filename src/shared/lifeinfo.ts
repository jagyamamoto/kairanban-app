// 生活情報(ごみの日・分別・防災)の「構造化された表示」。
//
// ============================================================
//  ⚠ 立ち上げのとき、このファイルは**触らなくて構いません**
// ============================================================
//
// 既定では生活情報タブは出ません(下の LIFE_INFO_ENABLED が false)。
// 中身は**すべて架空のサンプル**なので、書き換えずに出すと、
// よその町の嘘のごみの日が会員に表示されます。だから既定で切ってあります。
//
// ── ごみの日を載せたいとき、道は2つあります ──
//
//  A. 管理画面で書く(おすすめ・AIの契約は要りません)
//     「ページ」で新しいページを作り、日本語で書いて「翻訳を更新」を押すだけ。
//     やさしい日本語・英語・中国語・ベトナム語のタブが自動で出ます。
//     ファイルは1行も触りません。→ docs/セットアップ手順.md「ごみの日を載せる」
//
//  B. このファイルを書き換える(上級者向け)
//     区の多言語PDFまで一覧に並べたい町会だけ。5言語ぶん手で書くことになります。
//     AIコーディング道具があるなら、区の公式サイトを読ませて書かせるのが速いです。
//     → docs/AIに手伝ってもらう.md
//
// 書き換える場合の注意: リンクの https://www.example.com/ は
// すべて自治体の該当ページに差し替えること。地名・施設名も実在のものに。
import type { PubLang } from "./i18n";

/**
 * 生活情報タブを出すか。
 *
 * ⚠ 既定は false。**このファイルの中身を自分の町会のものに書き換えてから** true にすること。
 *   架空のごみの日を会員に見せると、ごみが出せない・収集されないという実害が出ます。
 *
 * ごみの日は管理画面の「ページ」でも載せられます(そちらはこの設定と関係なく使えます)。
 */
